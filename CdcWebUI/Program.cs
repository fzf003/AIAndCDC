    ...
    public static void Main(string[] args)
    {
        ...
        app.MapPost("/cdc", async (HttpContext ctx) =>
        {
            ...
            try
            {
                ...
                var database = root.GetProperty("source").GetProperty("db").GetString() ?? "Unknown";
                var table = root.GetProperty("source").GetProperty("table").GetString() ?? "Unknown";
                ...
                var after = root.TryGetProperty("after", out var a) && a.ValueKind != JsonValueKind.Null ? a : root.GetProperty("before");
                events.Enqueue(new CdcEvent(DateTime.Now, database,table, op, after.ToString()));
        ...
        app.MapGet("/api/events", () => Results.Json(events.OrderByDescending(e => e.Timestamp).Take(50)));
